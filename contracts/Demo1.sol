...
contract Demo{
    ...
    function confort(string calldata content) public OnlyLeader payable returns(bool){
        if(step != 2){
           revert(UNKNOW);
        }
    ...
    function confortRep(string calldata content) public NotLeader  returns(bool){
        if(step != 3){
           revert(UNKNOW);
        }
    ...
    function review(string calldata content,string memory correctContent) internal pure returns(bool){
        if( keccak256(abi.encodePacked(content)) == keccak256(abi.encodePacked(correctContent)) ){
            return true;
        }
        return false;
    }
